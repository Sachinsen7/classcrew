...
const productsData: Product[] = [
  {
    id: 1,
    title: "강의명강의명강의명강의명강의명강의명강의명강의명강의명강의명강의명강의명",
  ...
  {
    id: 2,
    title: "강의명강의명 강의명 강의명 강의명 강의명 강의명 강의명 강의명 강의명 강의명 강의명",
    thumbnail: "/shopping-basket.png",
        price: 10000,
    discount: 0,
  },
  {
    id: 3,
    title: "강의명 강의명 강의명 강의명 강의명 강의명 강의명 강의명 강의명 강의명 강의명 강의명",
    thumbnail: "/shopping-basket.png",
          price: 150000,
    discount: 15000,
    duration: "학습기간 : o개월",
  ...
  전체선택
        ...
        <div className="flex items-center gap-4">
          <label className="flex items-center gap-1 text-[20px] font-medium text-black">
            <input type="checkbox" className="w-[18px] h-[18px] accent-black" /> 과정만 보기
          </label>
          <label className="flex items-center gap-1 text-[20px] font-medium text-black">
            <input type="checkbox" className="w-[18px] h-[18px] accent-black"  /> 상품만 보기
          </label>
          <button className="border border-[rgba(0,0,0,0.50)] px-2 py-1 text-[20px] font-medium text-black flex  "> <X className="mt-1 w-[20px] h-[20px]"/> 선택삭제</button>
        </div>
      </div>
  ...
  >
    {selectedProducts.length === productsData.length
      ...
      : ""}
            ...
            <th className="p-2  text-[20px] font-bold text-black ">강의명/상품명</th>
            <th className="p-2  text-[20px] font-bold text-black">정가</th>
            <th className="p-2  text-[20px] font-bold text-black ">할인금액</th>
            <th className="p-2  text-[20px] font-bold text-black">총 결제금액</th>
          </tr>
        </thead>
        <tbody >
          {productsData.map((product) => {
            ...
            return (
              <tr key={product.id} className="border-b-2 border-[#D9D9D9] w-full">
                ...
                <td className="  flex items-start gap-3 m-10">
                  ...
                  />
                  <div className="">
                    <p className="w-[203px] text-[20px] font-medium ml-10 text-black">{product.title}</p>
                    {product.duration && (
                ...
                <td className="p-2 text-center text-[20px] font-medium text-black">{product.price.toLocaleString()}원</td>
                <td className="p-2  text-center text-[20px] font-medium text-black" >{product.discount.toLocaleString()}원</td>
      ...
      <div className="flex justify-end items-center mt-4 gap-4">
        <span className="text-[32px] font-medium text-black">
          총 <span className="font-bold">{selectedProducts.length}</span> 개 주문금액  <span className=" font-bold">{totalAmount.toLocaleString()} </span>원
        </span>
        <button className="bg-black text-white px-4 py-2 w-[199px] h-[69px] text-[24px] font-extrabold rounded">주문하기</button>
      </div>
    </div>
